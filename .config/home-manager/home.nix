{ config, pkgs, emacs-lsp-booster, lib, ... }:
  let
    ...
    # Vale's config format having no include directive.
    mergeValeIni = { base, extraDir }:
      let
        markerPrefix = "# EXTEND ";
        resolveLine = line:
          if lib.hasPrefix markerPrefix line
          then builtins.readFile (extraDir + "/${lib.removePrefix markerPrefix line}.ini")
          else line;
        lines = lib.splitString "\n" (builtins.readFile base);
      in lib.concatStringsSep "\n" (
        [
          "# GENERATED by home-manager from ${baseNameOf base} + ${baseNameOf extraDir};"
          "# do not edit this file directly."
          ""
        ]
        ++ map resolveLine lines
      );
